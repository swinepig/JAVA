...
import com.hengtian.zxjk.ZxFormat;
import com.hengtian.zxjk.ZxMsgFormats;
import com.hengtian.zxjk.ZxNode;

/**
 * 企业征信报文处理器
 * 
 * */
public class EnterpriseZXMsgProcessor {
	...
	private String newStr = "src/com/hengtian/zxjk/business/dom.xml";
	private String KONG = "                                                       ";
	...
	private String parseObjectToString(Object obj) throws Exception {
		if (null != obj) {
			...
			if ("-1".equals(zxFormat.getType())) {
				messageNumber = zxFormat.getChildren().size();
			}
	...
	private void processResult(String tempResult, StringBuffer sb,
		...
		// 并且此时传入的参数obj，就是被转化的原始obj
			...
			String foramtType = getZxFormatByClassName(obj.getClass().getName())
					.getType();
			// 被转化的obj为报文名对象
...
//				}
				...
				if (foramtType != null) {
					if (foramtType.equals("0")&& messageNumber!=1) {
						sb.append("\r\n");
						sb.append("\r\n");
						messageNumber--;
					} else if (foramtType.equals("2") || foramtType.equals("5")) {
						sb.append("\r\n");
					}
				}
			}
	...
	public Object parseTxtToObject(String nameMessage) {
		...
		try {
			...
			if (curZxFormat != null) {
				...
				// 报文名赋值
				// 报文名必须放在Children第一个
				ZxFormat zxFormatName = getZxFormatByClassName(curZxFormat
						.getChildren().get(0).getClassName());
				int beginOneName = 0;
				...
				// 报文头赋值
				// 报文头必须放在Children第二个
				ZxFormat zxFormatHead = getZxFormatByClassName(curZxFormat
						.getChildren().get(1).getClassName());
				int beginOneHead = 0;
				...
				if (lines.size() <= 1) {
					// 报文体必须放在Children第三个
					ZxFormat zxFormatBody = getZxFormatByClassName(curZxFormat
							.getChildren().get(2).getClassName());
					int beginOneBody = 0;
